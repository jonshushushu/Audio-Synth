...
public class AudioBuffer {
    ...
    /**
     * Renders a slice of the given signal into this audio buffer, adding the new signal to the sample
     * values already present in the buffer.
     *
     * @param signal   A sound source.
     * @param offset   The number of samples into the audio buffer to start adding the new signal. This
     *                 offset is time 0 for the signal.
     * @param duration The number of samples of the signal to add to the buffer. The offset + duration
     *                 must not exceed the length of the buffer.
     */
    public void mix(Signal signal, int offset, int duration) {
        for (int n = offset; n < (offset + duration) && n < samples.length; n++) {
            samples[n] = samples[n] + (float) signal.amplitudeAt(n - offset);
        }
    }
    ...
    /**
     * Plays this buffer to the system's default sound output. This method is _asynchronous_: it does
     ...
     *
     * @param playingCallback Called periodically during audio playback, indicating the current playback
     *                        position and whether the audio is done playing.
     */
    ...
    public String toString() {
        return "AudioBuffer [samples=" + Arrays.toString(samples) + "]";
    }
